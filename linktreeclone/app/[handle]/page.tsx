...
<section className=" bg-[#254f1a] mt-15 p-10 rounded-xl">
        <div className="flex flex-col items-center justify-center h-full  pb-5">
        <img className="rounded-lg" src={item.linkImage} alt="" />
        <h1 className="text-xl mt-2 font-bold">@{item.handle}</h1>
        <p className="max-w-60 text-slate-300 text-sm text-center">{item.desc}</p>
    </div>
    <div>
        {item && item.linkBox.map((linkBoxItem:any,index:any) => (
            <div className="bg-white hover:bg-gray-200 p-4 px-16 rounded-lg shadow-md mb-4" key={index}>
